...
const Reviews = ({navigation, route}) => {
  ...
  const ReviewCard = ({review}) => {
    const timestamp = review?.EntryDate
      ? parseInt(review.EntryDate.match(/\d+/)[0])
      : null;
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView
        ...
        {Data?.slice()
          .sort((a, b) => {
            const dateA = new Date(parseInt(a.EntryDate.match(/\d+/)[0]));
            const dateB = new Date(parseInt(b.EntryDate.match(/\d+/)[0]));
            return dateB - dateA;
          })
          .map((review, index) => (
